...
def git_commit_meta() -> tuple[str, str]:
    # Return placeholder metadata to prevent documentation drift in CI.
    # The actual commit hash/date will be inserted by the release process if needed,
    # but for repo consistency, we use a static token.
...
def html_logo_src(css_path: str) -> str:
    if css_path.startswith("../"):
        return "../../../assets/pvx_logo.png"
    return "../../assets/pvx_logo.png"


def html_attribution_href(css_path: str) -> str:
    if css_path.startswith("../"):
        return f"../../../{ATTRIBUTION_DOC_PATH}"
    return f"../../{ATTRIBUTION_DOC_PATH}"
...
def html_page(
    ...
    *,
    ...
    logo_src = html_logo_src(css_path)
    attribution_href = html_attribution_href(css_path)
    attribution_html = html_notice(attribution_href)
...
<body>
  <header class=\"site-header\">
    <div class=\"wrap\">
      <div class=\"brand\">
        <img class=\"pvx-logo\" src=\"{escape(logo_src)}\" alt=\"pvx logo\" />
        <h1>{escape(title)}</h1>
      </div>
      {breadcrumbs}
    </div>
