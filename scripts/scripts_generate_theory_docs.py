...
def compute_window_metrics(window: np.ndarray) -> dict[str, float]:
    ...
    return {
        "coherent_gain": round(float(coherent_gain), 10),
        "enbw_bins": round(float(enbw_bins), 10),
        "scalloping_loss_db": round(float(scalloping_loss_db), 10),
        "main_lobe_width_bins": round(float(main_lobe_width_bins), 10),
        "peak_sidelobe_db": round(float(peak_sidelobe_db), 10),
    }
