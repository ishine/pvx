...
def compute_window_metrics(window: np.ndarray) -> dict[str, float]:
    ...
    return {
        "coherent_gain": float(round(coherent_gain, 12)),
        "enbw_bins": float(round(enbw_bins, 12)),
        "scalloping_loss_db": float(round(scalloping_loss_db, 12)),
        "main_lobe_width_bins": float(round(main_lobe_width_bins, 12)),
        "peak_sidelobe_db": float(round(peak_sidelobe_db, 12)),
    }
