...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--detune-cents", type=float, default=14.0, help="Total detune span in cents"
    )
    parser.add_argument(
        "--width", type=float, default=1.0, help="Stereo width multiplier 0..2"
    )
    parser.add_argument(
        "--dry-mix", type=float, default=0.2, help="Dry signal mix amount"
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    for idx, path in enumerate(paths, start=1):
        try:
            ...
            voice_offsets = np.linspace(
                -args.detune_cents * 0.5, args.detune_cents * 0.5, num=args.voices
            )
            voice_offsets += rng.normal(
                scale=max(0.0, args.detune_cents * 0.04), size=args.voices
            )
            pans = (
                np.linspace(-1.0, 1.0, num=args.voices)
                * np.clip(args.width, 0.0, 2.0)
                * 0.8
            )
            ...
            log_message(
                args,
                f"[ok] {path} -> {out_path} | voices={args.voices}",
                min_level="verbose",
            )
    ...
    log_message(
        args,
        f"[done] pvxunison processed={len(paths)} failed={failures}",
        min_level="normal",
    )
