...
def build_transient_mask(
    ...
    *,
    ...
    if gap > 0:
        # Operating on region boundaries is much faster for dense masks than iterating all true indices.
        # We find starts and ends of True regions.
        # padded: [F, mask..., F] ensures we catch boundary transitions.
        # diff:   1 at start of True region, -1 at end (start of False region).
        diff = np.diff(np.concatenate(([False], mask, [False])).astype(np.int8))
        starts = np.flatnonzero(diff == 1)
        ends = np.flatnonzero(diff == -1)

        if ends.size > 1:
            # Distance between end of region i and start of region i+1.
            # ends[i] is the index after the last True of region i.
            # starts[i+1] is the index of the first True of region i+1.
            # The gap length in samples is starts[i+1] - ends[i].
            # The distance between the closest True samples is starts[i+1] - (ends[i] - 1) = starts[i+1] - ends[i] + 1.
            # The original logic checks: (next_true - prev_true) <= gap.
            # So here: starts[i+1] - (ends[i] - 1) <= gap.
            gaps = starts[1:] - ends[:-1] + 1
            to_bridge = np.flatnonzero(gaps <= gap)
            for idx in to_bridge:
                mask[ends[idx] : starts[idx + 1]] = True
